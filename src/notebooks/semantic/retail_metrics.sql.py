CREATE VIEW retail_project.retail_semantic.retail_metrics
WITH METRICS
LANGUAGE YAML
AS $$
version: 1.1

source: retail_project.retail_gold.fact_sales
comment: Retail metrics for analyzing sales transactions, revenue, and product performance

joins:
  - name: product
    source: retail_project.retail_gold.dim_product
    on: source.product_id = product.product_id

  - name: calendar
    source: retail_project.retail_gold.calendar
    on: source.transaction_date = calendar.date

  - name: customer
    source: retail_project.retail_gold.dim_customer
